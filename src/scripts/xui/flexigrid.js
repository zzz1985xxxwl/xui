...
(function ($) {
    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            toggleNode: function () {
                $(t).on("click", "td.node-parent a.toggle", function () {
                    var $this = $(this), td = $this.closest("td"), tbody = $this.closest("tbody"), isOpen = $this.hasClass("fa-minus-square"),
                        childTr = tbody.find("td[treeNode^=" + td.attr("treeNode") + "-" + "]").parent("tr");
                    if (isOpen) {
                    ...
                    $.each(data.rows, function (i, row) {
                        ...
                        $('thead tr:first th', g.hDiv).each( //add cell
                            function () {
                                ...
                                if (p.colModel[idx].type == "html") {
                                    td.innerHTML = $.xui.utils.template(p.colModel[idx].html, row.cell);
                                }
                                else if (typeof row.cell == 'undefined') {
                                ...
                                if (p.colModel[idx]['treeNode'] && row['treeNode']) {
                                    $(td).attr('treeNode', row['treeNode']).addClass("treeNode-p-" + (row['treeNode'].split('-').length - 1));
                                    if (i < datalength - 1 && data.rows[i + 1].treeNode.indexOf(row['treeNode']) >= 0) {
                                        $(td).addClass("node-parent").prepend("<a class='toggle fa-minus-square'></a>");
                                    }
                                }
            ...
            populate: function () { //get latest data
                ...
                var param =   {
                    ...
                    'query': p.query,
                    'qtype': p.qtype,
                    'Orderby': p.sortname + '  ' + p.sortorder
                };

                if (typeof (p.params) === "function") {
                    p.params= p.params();
                }
                if (p.params.length) {
...
//                g.cdpad += (isNaN(parseInt($(cdcol).css('paddingRight'), 10)) ? 0 : parseInt($(cdcol).css('paddingRight'), 10));
        ...
        if (p.title) {
            ...
            if (p.showTableToggleBtn) {
                $(g.mDiv).append('<div class="ptogtitle" title="Minimize/Maximize Table"><i class="fa-angle-down"></i></div>');
